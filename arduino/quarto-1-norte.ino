//#define NODE_ID '1'  // this is the Q1N node
extern const uint8_t node_id = 0xf8;
...
HT1w q1_north_ambient_sensor;
HT1w q1_north_vent_sensor;
HT1w q1_north_wall_sensor;
HT1w q1_east_wall_sensor;

void setup() {
  q1_north_ambient_sensor.begin(4, 20000, 0, 5);
  q1_north_vent_sensor.begin(7, 20000, 3000);
  q1_north_wall_sensor.begin(8, 20000, 6000);
  q1_east_wall_sensor.begin(9, 20000, 9000);
  msg_begin();
}
...
void loop() {
  ...
  q1_north_ambient_sensor.verify();
  q1_north_vent_sensor.verify();
  q1_north_wall_sensor.verify();
  q1_east_wall_sensor.verify();
  heartbeat_verify();

  verify_comm();

  uint8_t nsensors = 0;
  if (q1_north_ambient_sensor.hasNewData()) nsensors += 2;
  if (q1_north_vent_sensor.hasNewData()) nsensors += 2;
  if (q1_north_wall_sensor.hasNewData()) nsensors += 2;
  if (q1_east_wall_sensor.hasNewData()) nsensors += 2;
  ...
  if (msg_start()) {
    ...
    if (q1_north_ambient_sensor.hasNewData()) {
      msg_putbyte(0);
      msg_putword(q1_north_ambient_sensor.rawHumidity());
      msg_putbyte(1);
      msg_putword(q1_north_ambient_sensor.rawTemperature());
    }
    if (q1_north_vent_sensor.hasNewData()) {
      msg_putbyte(2);
      msg_putword(q1_north_vent_sensor.rawHumidity());
      msg_putbyte(3);
      msg_putword(q1_north_vent_sensor.rawTemperature());
    }
    if (q1_north_wall_sensor.hasNewData()) {
      msg_putbyte(4);
      msg_putword(q1_north_wall_sensor.rawHumidity());
      msg_putbyte(5);
      msg_putword(q1_north_wall_sensor.rawTemperature());
    }
    if (q1_east_wall_sensor.hasNewData()) {
      msg_putbyte(6);
      msg_putword(q1_east_wall_sensor.rawHumidity());
      msg_putbyte(7);
      msg_putword(q1_east_wall_sensor.rawTemperature());
    }
    if (hasNewHeartbeat) {
      msg_putbyte(255);
    ...
    if (msg_end()) {
      if (q1_north_ambient_sensor.hasNewData()) q1_north_ambient_sensor.dataIsOld();
      if (q1_north_vent_sensor.hasNewData()) q1_north_vent_sensor.dataIsOld();
      if (q1_north_wall_sensor.hasNewData()) q1_north_wall_sensor.dataIsOld();
      if (q1_east_wall_sensor.hasNewData()) q1_east_wall_sensor.dataIsOld();
